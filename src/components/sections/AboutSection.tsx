      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 w-full flex flex-col items-center gap-12 lg:gap-16">
        ...
        {about.moreAboutMe && about.moreAboutMe.length > 0 && (
          <div className="w-full flex justify-center py-4">
            <button 
              onClick={() => setIsMoreAboutMeOpen(!isMoreAboutMeOpen)}
            ...
            >
              <span>{isMoreAboutMeOpen ? 'Show less' : 'More about me'}</span>
              <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${isMoreAboutMeOpen ? 'rotate-180' : 'group-hover:translate-y-0.5'}`} />
            </button>
          </div>
        )}
